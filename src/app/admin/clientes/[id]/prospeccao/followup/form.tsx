...
export function FollowupProspeccaoForm({
  organizationId,
  regrasIniciais,
}: {
  ...
  function salvar() {
    ...
    startTransition(async () => {
      const result = await salvarProspeccaoConfig(organizationId, {
        followup: regras,
      });
      if ("error" in result) {
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-cinza-medio mt-1">
              Quando um lead de prospecção responde uma vez e depois some,
              essas mensagens entram em ação. O tom é diferente do follow-up
              de leads inbound — aqui você iniciou o contato e ele
              respondeu, então a retomada precisa lembrar disso.
            </p>
          </div>
          <button
            type="button"
        ...
        {regras.length === 0 ? (
          <p className="text-sm text-cinza-medio text-center py-8 bg-offwhite rounded-lg border border-cinza-claro">
            Nenhuma regra. Quando um lead de prospecção sumir após responder,
            ele vai direto pra &quot;perdido&quot; sem novas mensagens.
          </p>
        ) : (
